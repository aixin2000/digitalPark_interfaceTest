...
@allure.epic("小昆山获取招商服务产品列表测试case")
@allure.feature('小昆山获取招商服务产品列表功能')
class Test_Product_List:
    @allure.story('小昆山获取招商服务产品列表接口')
    @allure.severity('blocker')
    @allure.description("这里是对小昆山获取招商服务产品列表用例的一些详细说明")
    ...
    @pytest.mark.run(order=3)  # x是整数(可以是正数也可以是负数)
    # 执行失败后重试， 重试3次，每次重试间隔2秒
    # @pytest.mark.flaky(reruns=3, reruns_delay=2)
    @pytest.mark.parametrize("args_name", read_testcase('\\product_list_data.yaml'))  ##读取测试用例的product_list_data.yaml文件中的参数及值，赋值给变量args_name
    def test_product_list(self, args_name):
        res = RequestUtil("base_test_url", Test()).standard_yaml(args_name)


if __name__ == '__main__':
    pytest.main(['-vs', './test_product_list.py'])