...
@allure.epic("小昆山园区企业详情测试case")
@allure.feature('小昆山园区企业详情功能')
class Test_Enterprise_Details:
    @allure.story('小昆山园区企业详情接口')
    @allure.severity('blocker')
    @allure.description("这里是对小昆山园区企业详情用例的一些详细说明")
    ...
    @pytest.mark.parametrize("args_name", read_testcase(
        '\\enterprise_details_data.yaml'))  ##读取测试用例的enterprise_details_data.yaml文件中的参数及值，赋值给变量args_name
    def test_enterprise_details(self, args_name):
        res = RequestUtil("base_test_url", Test()).standard_yaml(args_name)


if __name__ == '__main__':
    pytest.main(['-vs', './test_enterprise_details.py'])
