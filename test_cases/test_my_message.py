...
@allure.epic("小昆山我的消息模块测试case")
@allure.feature('小昆山我的消息模块功能')
class Test_My_Message:
    @allure.story('小昆山我的消息模块接口')
    @allure.severity('blocker')
    @allure.description("这里是对小昆山我的消息模块用例的一些详细说明")
    ...
    @pytest.mark.parametrize("args_name", read_testcase('\\my_message_data.yaml'))  ##读取测试用例的product_list_data.yaml文件中的参数及值，赋值给变量args_name
    def test_my_message(self, args_name):
        res = RequestUtil("base_test_url", Test()).standard_yaml(args_name)


if __name__ == '__main__':
    pytest.main(['-vs', './test_my_message.py'])